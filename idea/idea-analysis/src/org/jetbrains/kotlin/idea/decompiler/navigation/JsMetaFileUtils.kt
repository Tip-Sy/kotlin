/*
 * Copyright 2010-2015 JetBrains s.r.o.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.jetbrains.kotlin.idea.decompiler.navigation

import com.intellij.openapi.vfs.VfsUtilCore
import com.intellij.openapi.vfs.VirtualFile
import org.jetbrains.kotlin.idea.decompiler.KotlinJavascriptMetaFileType
import org.jetbrains.kotlin.load.kotlin.PackageClassUtils
import org.jetbrains.kotlin.name.ClassId
import org.jetbrains.kotlin.name.FqName
import kotlin.platform.platformStatic

public object JsMetaFileUtils {
    platformStatic
    public fun getPackageFqName(file: VirtualFile): FqName = getPackageFqName(getRelativeToRootPath(file))

    platformStatic
    public fun getClassFqName(file: VirtualFile): FqName = getClassFqName(getRelativeToRootPath(file))

    platformStatic
    public fun getClassId(file: VirtualFile): ClassId = getClassId(getRelativeToRootPath(file))

    platformStatic
    public fun isPackageHeader(file: VirtualFile): Boolean = isPackageHeader(getRelativeToRootPath(file))

    private fun getRelativeToRootPath(file: VirtualFile): String {
        val root = getRoot(file)
        return VfsUtilCore.getRelativePath(file, root)!!
    }

    private val DEFAULT_PACKAGE_METAFILE_NAME = PackageClassUtils.DEFAULT_PACKAGE_CLASS_NAME + "." + KotlinJavascriptMetaFileType.INSTANCE.getDefaultExtension()

    private fun getClassFqName(relPath: String): FqName {
        val index = relPath.indexOf(VfsUtilCore.VFS_SEPARATOR_CHAR)
        val pathToFile = relPath.substring(index+1)

        val indexOfLastPoint = pathToFile.lastIndexOf('.')
        val name = if (indexOfLastPoint >= 0) pathToFile.substring(0, indexOfLastPoint) else pathToFile
        return FqName(name.replace('/', '.'))
    }

    private fun getClassId(relPath: String): ClassId {
        val classFqName = getClassFqName(relPath)
        val shortName = classFqName.shortName().asString()
        val indexOfLastPoint = shortName.lastIndexOf('.')
        val name = if (indexOfLastPoint >= 0) shortName.substring(0, indexOfLastPoint) else shortName
        val packageFqName = getPackageFqName(relPath)
        return ClassId(packageFqName, FqName(name), false)
    }

    private fun getPackageFqName(relPath: String): FqName {
        val index = relPath.indexOf(VfsUtilCore.VFS_SEPARATOR_CHAR)
        val pathToFile = relPath.substring(index+1)
        if (pathToFile == DEFAULT_PACKAGE_METAFILE_NAME) return FqName.ROOT

        val indexOfLastSeparator = pathToFile.lastIndexOf(VfsUtilCore.VFS_SEPARATOR_CHAR)
        val name = if (indexOfLastSeparator >= 0) pathToFile.substring(0, indexOfLastSeparator) else pathToFile
        return FqName(name.replace(VfsUtilCore.VFS_SEPARATOR_CHAR, '.'))
    }

    private fun isPackageHeader(relPath: String): Boolean {
        val classFqName = JsMetaFileUtils.getClassFqName(relPath)
        return PackageClassUtils.isPackageClassFqName(classFqName)
    }

    private fun getRoot(file: VirtualFile): VirtualFile {
        if (file.getParent() == null) return file else return getRoot(file.getParent())
    }
}

